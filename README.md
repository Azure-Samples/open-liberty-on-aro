...
Finish the following prerequisites in order to successfully walk through this guide.

1. Install JDK per your needs (e.g., [AdoptOpenJDK OpenJDK 8 LTS/OpenJ9](https://adoptopenjdk.net/?variant=openjdk8&jvmVariant=openj9)).
...
Open Liberty is a Jakarta EE 8/Java EE 8 full profile compatible app server.  If you already have a Jakarta EE/Java EE application running on an existing app server (for example, IBM WebSphere Network Deployment, Oracle WebLogic Server, JBoss EAP, etc.), only minimal changes are necessary to make it run on Open Liberty.

### Get a Quick Start with A basic Jakarta EE/Java EE App
...
3. Wait until the server starts. You will output similar to the following in your console.
...
For reference, these have already been applied in `<path-to-repo>/2-simple` of your local clone.
...
5. Copy and paste the contents of above yaml file or start from the default yaml auto-generated, the final yaml should look like the following.
...
Once the Open Liberty Application is running and its route is created, you can visit the application home page by opening the URL copied from value of "HOST/PORT" from the route in your browser.
